...
from loss.dice_loss import GDiceLossV2,SoftDiceLoss,GDiceLoss
...
def train(data_dir,ckpt_path,save_path,epoch_num,label):
    ...
    train_set = CULane(path = data_dir, image_set='train')
    val_set = CULane(path = data_dir, image_set='val')
    ...
    for epoch in range(num_epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            '''discriminative loss'''
            embedding_loss = discriminative_loss(embeddings,labels_inst,delta_v=0.2,delta_d=1,param_var=.5,
                                                param_dist=.5,param_reg=0.001)

            loss = bin_loss  + embedding_loss * 0.3
            ...
            if step % 1000 == 0 :
                ...
                ckpt_path = os.path.join(ckpt_dir, 'ckpt_%s_step-%d.pth' % (label, step))
                torch.save({
                    'epoch': epoch,
                    'step': step,
...
def val(model, data_loader, epoch_idx, device, summerwriter ,label):
